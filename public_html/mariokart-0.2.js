    ...
    function MarioKart()
    {
        ...
        function resetScreen()
        {
            ...
            for (var iViewY = 0; iViewY < iHeight; iViewY += fLineScale)
            {
                ...
                // total height of point (on view) from the ground up

                var iDeltaY = iCamHeight - iTotalY;
                // height of point relative to camera

                var iPointZ = (iTotalY / (iDeltaY / iCamDist));
                // distance to point on the map
        ...
        var iViewCanvasHeight = 90;
        // these height, width and y-offset values

        var iViewCanvasWidth = 256;
        // have been adjusted to work with the current camera setup
        ...
        function Sprite(strSprite)
        {
            ...
            this.draw
                = function(iX, iY, fScale)
                  {

                  var bDraw = true;

                  if (iY > iHeight * iScreenScale
                      || iY < 6 * iScreenScale)
                  {
            ...
            for (var i = 0; i < aStrips.length; i++)
            {
                ...
                if (iRenderMode == 0)
                {
                    ...
                    try
                    {
                        ...
                        (
                            ...
                            ( iHeight - oStrip.viewy) / fLineScale,
                ...
                if (iRenderMode == 1)
                {
                    ...
                    oStrip
                        .canvas
                        .getContext("2d")
                        .clearRect(0, 0, oStrip.stripwidth, 1);
        ...
        function canMoveTo(iX, iY)
        {
            ...
            if (iX > iMapWidth - 5 || iY > iMapHeight - 5)
                return false;
            ...
            if (!oKart.cpu)
            {
                ...
                else
                {
                    ...
                    (
                        ...
                        oKart.rotinc == -fMaxRotInc
                            && fMaxRotTimer > 0
                            && (new Date().getTime() - fMaxRotTimer)
                            > 800
        ...
        function ai(oKart)
        {
            ...
            iRotatedX
                = iLocalX
                * Math.cos(oKart.rotation * Math.PI / 180)
                -  iLocalY
                * Math.sin(oKart.rotation * Math.PI / 180);
        ...
        function selectMapScreen()
        {
            ...
            for (var i = 0; i < aAvailableMaps.length; i++)
            {
                ...
                oPImg.style.left =
                    ( ( (iWidth - 30 * aAvailableMaps.length)
                        / 2
                        + i
                        * 30
                        +  i)
    ...
    // 1->IE, 0->FF, 2->GCrome, 3->Safari, 4->Opera, 5->Konqueror
        ...
        b = (typeof/./)[0]
            ...
            +'1\0'
                ?5:1-'\0'?1
                :
            +{valueOf:function(x){return!x}}?4:0;


    if( (typeof/./) [0]
        == 'f'
        && parseFloat((sAgent.match(/(?:firefox|minefield)\/([\d\.]+)/i) || {})[1])
        <= 2)
            b = 0;
        ...
        if (func && load_events.length == 1)
        {
            ...
            if (doc.addEventListener && !isOpera)
            {
                ...
                window.addEventListener
                    ("DOMContentLoaded", load_init, false);
